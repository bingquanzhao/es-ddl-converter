...
CREATE TABLE IF NOT EXISTS `my_logs` (
    ...
    `user_id`        BIGINT              NULL        COMMENT 'long',
    `user_name`      VARCHAR(256)        NULL        COMMENT 'keyword',
    ...
    `_extra`         VARIANT             NULL        COMMENT 'dynamic=true, catch-all for unmapped fields',
    ...
    INDEX idx_user_name(`user_name`) USING INVERTED COMMENT 'keyword exact match'
)
...
- `object` 嵌套结构递归展平（`user.id` → `user_id`，`user.name` → `user_name`）
- `text` + `.keyword` multi-fields 合并为单列，不重复建列
- `dynamic: true` 自动追加 `_extra VARIANT` 兜底列
- keyword / text 列自动生成倒排索引（含 analyzer → parser 映射）
- Key 列、分区、分桶策略自动推断
...
| dynamic mapping | 追加 `_extra VARIANT` | 兜底未映射字段 |
...
| `ik_max_word` / `ik_smart` / `smartcn` | `chinese` |
