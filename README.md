...
table:
  ...
  flatten_fields: []        # 需展平为独立列的 object 字段路径列表（默认映射为 VARIANT）
...
| `date` | `DATE` / `DATETIME(0)` / `DATETIME(3)` | 纯日期格式→`DATE`，epoch_second→`DATETIME(0)`，其他→`DATETIME(3)` |
...
| `ip_range` | `{name}_gte VARCHAR(64)`，`{name}_lte VARCHAR(64)` |
...
| `object`（有子属性） | `VARIANT`（默认） | 通过 `flatten_fields` 可按需展平为 `user_name` 等独立列 |
| `object`（`enabled: false`） | `VARIANT` | 保留原始结构，不解析子字段 |
| `nested` | `VARIANT` | 丢失嵌套关联语义，输出告警 |
| `flattened` | `VARIANT` | — |
| `dense_vector` | `ARRAY<FLOAT>` | 无 ANN 检索能力 |
| `aggregate_metric_double` | 按 metrics 展开多列 | 如 `{name}_min DOUBLE`、`{name}_max DOUBLE`、`{name}_count BIGINT` |
...
| `smartcn` | `chinese` |
| 其他 / 无 | `unicode`（未识别的自定义 analyzer 会输出告警） |
